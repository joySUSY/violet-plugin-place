...
# High-Context Code Commenting | 高上下文代码注释

Principles for high-context idiomatic comments in Rust. | 在 Rust 中记录高上下文惯用注释的原则。

## 🔬 Research & Modernity (2026): Semantic Logic Capture | 研究与现代性 (2026)：语义逻辑捕获

- **Inferred Exception Documentation**: Document not just what a function returns, but the "Why" behind potential panic conditions based on 2026 AI-augmented logic audits. | **推断异常文档**: 不仅记录函数返回的内容，还要根据 2026 年 AI 增强逻辑审计记录潜在 panic 条件背后的“原因”。
- **Human-Centric Clarity Markers**: Use specific markers to distinguish between "Machine-Verified" logic and "Human-Intent" heuristics. | **以人为中心的清晰度标记**: 使用特定标记来区分“机器验证”逻辑和“人类意图”启发式。
