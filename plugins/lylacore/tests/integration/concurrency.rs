...
async fn test_no_race_conditions_key_derivation() {
    ...
    for _ in 0..50 {
        let salt_clone = salt.clone();
        set.spawn(async move {
            lyl_cipher::derive_key("password", &salt_clone, KeyDerivationAlgorithm::Argon2id)
                .await
                .expect("Key derivation failed")
        });
    }
...
async fn test_mixed_operations_concurrently() {
    ...
    for i in 0..5 {
        set.spawn(async move {
            let salt = generate_salt();
            let passphrase = format!("password-{}", i);
            lyl_cipher::derive_key(&passphrase, &salt, KeyDerivationAlgorithm::Argon2id)
                .await
                .expect("Key derivation failed");
        });
    }
...
async fn test_concurrent_error_handling() {
    ...
    let wrong_key = lyl_cipher::derive_key("wrong", &wrong_salt, KeyDerivationAlgorithm::Argon2id)
        .await
        .expect("Key derivation failed");
