...
async fn test_empty_passphrase_error() {
    ...
    let result = lyl_cipher::derive_key("", &salt, KeyDerivationAlgorithm::Argon2id).await;
...
async fn test_authentication_failure() {
    ...
    let key1 = lyl_cipher::derive_key("password1", &salt1, KeyDerivationAlgorithm::Argon2id)
        .await
    ...
    let key2 = lyl_cipher::derive_key("password2", &salt2, KeyDerivationAlgorithm::Argon2id)
        .await
        .expect("Key derivation failed");
...
fn test_corrupted_auth_tag() {
    ...
    let corrupted_tag =
        lyl_cipher::AuthTag::from_slice(&corrupted_tag_bytes[..]).expect("Failed to create tag");
...
async fn test_unsupported_algorithm_error() {
    ...
    for algo in [
        ...
        let result = lyl_cipher::derive_key("password", &salt, algo).await;
...
fn test_error_message_contains_context() {
    ...
    let result = lyl_cipher::Salt::from_slice(&[0u8; 16][..]);
    assert!(result.is_err());
...
async fn test_extremely_long_passphrase() {
    ...
    let result =
        lyl_cipher::derive_key(&long_passphrase, &salt, KeyDerivationAlgorithm::Argon2id).await;
