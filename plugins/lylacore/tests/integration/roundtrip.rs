...
use lyl_cipher::{decrypt, encrypt, generate_salt, KeyDerivationAlgorithm};

// Helper functions
fn salt_from_hex(hex: &str) -> lyl_cipher::Salt {
    let bytes = hex::decode(hex).expect("Invalid hex");
    lyl_cipher::Salt::from_slice(bytes.as_slice()).expect("Invalid salt size")
}
...
async fn test_rust_encrypt_rust_decrypt() {
    ...
    let key = lyl_cipher::derive_key("password", &salt, KeyDerivationAlgorithm::Argon2id)
        .await
...
async fn test_deterministic_key_derivation() {
    ...
    let key1 = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Argon2id)
        .await
        .expect("Key derivation failed");

    let key2 = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Argon2id)
        .await
        .expect("Key derivation failed");
...
async fn generate_cross_language_test_vectors() {
    ...
    let key_argon2id = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Argon2id)
        .await
        .expect("Key derivation failed");
    ...
    let key_pbkdf2 = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Pbkdf2)
        .await
        .expect("Key derivation failed");
    ...
    let key_scrypt = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Scrypt)
        .await
        .expect("Key derivation failed");
...
async fn test_all_algorithms_produce_different_keys() {
    ...
    let key_argon2 = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Argon2id)
        .await
...
async fn test_key_isolation_between_users() {
    ...
    let key1 = lyl_cipher::derive_key("user1-password", &salt1, KeyDerivationAlgorithm::Argon2id)
        .await
        .expect("Key derivation failed");

    let key2 = lyl_cipher::derive_key("user2-password", &salt2, KeyDerivationAlgorithm::Argon2id)
        .await
        .expect("Key derivation failed");
...
async fn test_workflow_derive_encrypt_decrypt() {
    ...
    // Step 1: Derive key
    let key = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Argon2id)
        .await
        .expect("Key derivation failed");
