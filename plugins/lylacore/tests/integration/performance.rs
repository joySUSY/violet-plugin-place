...
async fn test_key_derivation_performance() {
    ...
    for _ in 0..3 {
        let _ = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Argon2id)
            .await
...
async fn test_algorithm_performance_comparison() {
    ...
    // PBKDF2
    let start = Instant::now();
    let _ = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Pbkdf2)
        .await
    ...
    // Scrypt
    let start = Instant::now();
    let _ = lyl_cipher::derive_key(passphrase, &salt, KeyDerivationAlgorithm::Scrypt)
        .await
