...
# Security Core & Vulnerability Auditing | 安全核心与漏洞审计

Comprehensive application security guidance for TypeScript, Python, and Rust applications.

## 🔬 Research & Modernity (2026): Proactive Defense | 研究与现代性 (2026)：主动防御

- **Semantic Secret Detection**: Using AI-augmented regex patterns to identify high-entropy strings that likely represent API keys or certificates. | **语义密钥检测**: 使用 AI 增强的正则表达式模式来识别极有可能代表 API 密钥或证书的高熵字符串。
- **Software Bill of Materials (SBOM)**: Patterns for generating and verifying CyclonDX/SPDX manifests to audit the supply chain. | **软件物料清单 (SBOM)**: 生成和验证 CyclonDX/SPDX 清单以进行供应链审计的模式。
