...
# PMAT# Multi-Language Project Analysis (PMAT) | 多语言项目分析 (PMAT)
Formal protocols for analyzing complex, polyglot codebases using PMAT (Polyglot Mapping & Analysis Tool). | 使用 PMAT（多语言映射与分析工具）分析复杂、多语言代码库的正式协议。

## 🔬 Research & Modernity (2026): Cross-Layer Analytics | 研究与现代性 (2026)：跨层分析

- **Inter-Language Dependency Extraction**: Using PMAT 2026 engines to trace logic flow across FFI boundaries (e.g., Rust to Java). | **语言间依赖提取**: 使用 PMAT 2026 引擎跟踪跨 FFI 边界（例如 Rust 到 Java）的逻辑流。
- **Context-Aware Code Graphing**: Automated generation of polyglot dependency graphs that distinguish between compile-time and runtime dependencies. | **上下文感知代码图谱**: 自动生成区分编译时和运行时依赖关系的多语言依赖图。
