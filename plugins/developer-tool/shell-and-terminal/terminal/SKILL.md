...
# terminal
# Terminal Mastery & Shell Environment | 终端精通与 Shell 环境

Advanced techniques for terminal interaction and shell environment management. | 终端交互和 Shell 环境管理的高级技术。
...
## 🔬 Research & Modernity (2026): Performance & Integration | 研究与现代性 (2026)：性能与集成

- **GPU-Accelerated Rendering**: Optimizing terminal throughput for Ghostty and WezTerm via specialized escape sequences. | **GPU 加速渲染**: 通过专用的转义序列优化 Ghostty 和 WezTerm 的终端吞吐量。
- **Workspace-Aware Shells**: Auto-injecting workspace context7 data into every sub-shell session via `.zshrc` hooks. | **工作区感知 Shell**: 通过 `.zshrc` 钩子将工作区 context7 数据自动注入到每个子 shell 会话中。
